 ...
 */
export function rateFor(userId, year) {
  ...
  if (cfg && cfg.inherits && p.rates[cfg.inherits]) {
    return { rate: p.rates[cfg.inherits], basis: "inherited", year: cfg.inherits };
  }
  ...
  if (past.length) {
    ...
    return { rate: p.rates[k], basis: "carried", year: k };
  }
...
export async function fetchProjectCosts() {
  ...
  const sql =
    ...
    `YEAR("${c.date}") AS yr, SUM("${c.hours}")*1 AS hours, ` +
    `SUM("${c.hours}"*"${rate}")*1 AS zoho_cost, ` +
    `SUM(CASE WHEN "${rate}" > 0 THEN 0 ELSE "${c.hours}" END)*1 AS unrated ` +
    `FROM "${TIME_LOGS}" GROUP BY "${c.project}", "User ID", YEAR("${c.date}")`;
  ...
  for (const r of parseCsv(await runCostQueryRaw(sql))) {
    ...
    const { rate: hourly, basis } = rateFor(uid, y);
    ...
    const e = map[pid] || (map[pid] = { hours: 0, cost: 0, unrated: 0, byYear: {}, basis: {} });
    ...
    if (y) {
      ...
      b.hours += hours; b.cost += cost;
      if (priced === "none") b.unrated += hours;
    }
  }
  return map;
}
...
export function assemble(invoices, projects, costs, deals, errors, opts) {
  ...
  for (const p of projects) {
    ...
    projRows.push({
      ...
      unrated: cst ? round(cst.unrated || 0) : null,
