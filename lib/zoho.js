...
/* ------------------------------------------------- flag billable dei log -- */

/**
 * La regola, in un posto solo.
 *
 * Un progetto è interno o di sviluppo se il nome comincia per _ = == --- o ::
 * Le sue ore non si fatturano mai. Su un progetto cliente si fattura tutto,
 * tranne le ore registrate sotto una tasklist o su un task il cui nome comincia
 * per _ : è la convenzione con cui marchiamo il lavoro interno dentro un
 * progetto cliente.
 *
 * Vive qui e non dentro la query perché la stessa regola serve a due cose —
 * trovare i log sbagliati e decidere il Billing Type dei task — e due copie
...
/** Billable o no, secondo la regola. */
export function targetBillStatus(projectName, tasklistName, taskName) {
  if (isInternalProjectName(projectName)) return "Non Billable";
  const marked = (s) => String(s || "").trim().startsWith("_");
  if (marked(tasklistName) || marked(taskName)) return "Non Billable";
  return "Billable";
}

// In SQL la stessa cosa, perché il confronto conviene farlo dove stanno i dati.
const SQL_INTERNAL = (col) =>
  `LEFT(TRIM(REPLACE(${col},'"','')),1) IN ('_','=',':','-')`;

/**
 * I log che non rispettano la regola, con tutto quello che serve per correggerli.
 *
 * Le due grafie di Zoho — "Non Billable" e "NonBillable" senza spazio, che
 * convivono dal giugno 2026 — vengono normalizzate qui: un filtro che ne
 * conosce una sola perde silenziosamente l'altra.
 */
export async function billingOffenders(opts) {
  const o = opts || {};
  const from = Number(o.from) || 2025;
  const to = Number(o.to) || 2026;
  const sql =
    `SELECT CONCAT(L."Log ID",'') AS log_id, CONCAT(L."Project ID",'') AS project_id, ` +
    `P."Project Name" AS project, CONCAT(L."Task ID",'') AS task_id, ` +
    `CONCAT(L."Bug ID",'') AS bug_id, L."Type" AS type, L."Status" AS current, ` +
    `L."Hours"*1 AS hours, DATE_FORMAT(L."Date",'%Y-%m-%d') AS day, ` +
    `CASE WHEN ${SQL_INTERNAL('P."Project Name"')} THEN 'Non Billable' ` +
    `WHEN LEFT(TRIM(T."Tasklist Name"),1) = '_' OR LEFT(TRIM(T."Task Name"),1) = '_' ` +
    `THEN 'Non Billable' ELSE 'Billable' END AS target ` +
    `FROM "${TIME_LOGS}" L ` +
    `LEFT JOIN "Projects (Zoho Projects)" P ON P."Project ID" = L."Project ID" ` +
    `LEFT JOIN "${TASKS}" T ON T."Task ID" = L."Task ID" ` +
    `WHERE YEAR(L."Date") BETWEEN ${from} AND ${to} ` +
    `AND REPLACE(L."Status",' ','') <> ` +
    `REPLACE(CASE WHEN ${SQL_INTERNAL('P."Project Name"')} THEN 'Non Billable' ` +
    `WHEN LEFT(TRIM(T."Tasklist Name"),1) = '_' OR LEFT(TRIM(T."Task Name"),1) = '_' ` +
    `THEN 'Non Billable' ELSE 'Billable' END,' ','')`;

  return parseCsv(await runCostQueryRaw(sql)).filter((r) => r.log_id);
}

/**
 * Riscrive il flag sui log. In prova non tocca niente e dice cosa farebbe.
 *
 * Si lavora a lotti: la funzione serverless ha 300 secondi e ogni log è una
 * chiamata a Zoho. Il report dice sempre quanti ne restano, così si richiama
 * finché non è zero invece di sperare che ci stiano tutti in un colpo.
 */
export async function applyBillingFix(opts) {
  const o = opts || {};
  const portal = ENV("ZOHO_PROJECTS_PORTAL_ID", "20070118907");
  const limit = Math.max(1, Math.min(Number(o.limit) || 400, 2000));
  const deadline = Date.now() + (Number(o.seconds) || 200) * 1000;

  const all = await billingOffenders(o);
  const out = {
    rule: "internal projects (_ = == --- ::) and _ tasklists/tasks are Non Billable, the rest Billable",
    years: [Number(o.from) || 2025, Number(o.to) || 2026],
    apply: !!o.apply,
    to_change: all.length,
    by_direction: {
      to_billable: all.filter((r) => r.target === "Billable").length,
      to_non_billable: all.filter((r) => r.target !== "Billable").length,
    },
    written: 0, failed: 0, skipped: 0, remaining: all.length, log: [],
  };

  if (!o.apply) {
    out.sample = all.slice(0, 25).map((r) => ({
      project: r.project, day: r.day, type: r.type,
      hours: Number(r.hours) || 0, from: r.current, to: r.target,
    }));
    return out;
  }

  for (const r of all) {
    if (out.written + out.failed >= limit || Date.now() > deadline) break;

    // Un log generico non sta sotto un task né sotto un bug: l'API non offre un
    // percorso per aggiornarlo, quindi si dichiara invece di fallire in silenzio.
    const base = `https://projectsapi.zoho.eu/restapi/portal/${portal}/projects/${r.project_id}`;
    const path = r.task_id && r.task_id !== "0" ? `/tasks/${r.task_id}/logs/${r.log_id}/`
               : r.bug_id && r.bug_id !== "0" ? `/bugs/${r.bug_id}/logs/${r.log_id}/`
               : null;
    if (!path) {
      out.skipped += 1;
      out.log.push({ project: r.project, day: r.day, log_id: r.log_id,
                     status: "skipped — general log, no API path to update it" });
      continue;
    }

    try {
      await zoho(base + path, {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: new URLSearchParams({ bill_status: r.target }),
      });
      out.written += 1;
    } catch (e) {
      out.failed += 1;
      out.log.push({ project: r.project, day: r.day, log_id: r.log_id,
                     from: r.current, to: r.target,
                     status: "failed — " + e.message.slice(0, 160) });
    }
  }

  out.remaining = all.length - out.written - out.skipped;
  out.note = out.remaining > 0
    ? `Call again with the same parameters to continue: ${out.remaining} still to change.`
    : "Nothing left to change. Analytics refreshes on its own schedule, so a re-run may still " +
      "list these until the next sync.";
  return out;
}
