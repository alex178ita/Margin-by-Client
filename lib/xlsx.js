...
function marginPctBlock(ws, revenue, cost, hours, extraNote) {
  ...
  note(ws, "Hours are broken down person by person on the team sheet. What is not in this file is " +
           "the money behind them: hourly costs and the cost of the team are part of the internal " +
           "costs view.");
  if (extraNote) note(ws, extraNote);
}
...
/**
 * Le ore per persona senza il denaro.
 *
 * Chi non ha sbloccato i costi interni vede comunque chi ha lavorato e quanto:
 * sono i log, non i cedolini. Quello che sparisce è la sola colonna che traduce
 * un'ora in euro — tariffa individuale e costo generato — e con essa la
 * possibilità di risalire a quanto costa una singola persona.
 *
 * Resta la quota di ore sul progetto, che è la domanda vera di chi legge senza
 * costi: chi ci ha messo le mani, e per quanta parte del lavoro.
 */
function teamHoursBlock(ws, people, heading) {
  band(ws, heading || "HOURS PER PERSON (Time Logs, Zoho Projects)");
  const rows = people.map((p) => ({
    warn: p.unrated > 0,
    cells: [p.person, p.role || "—", p.logs, p.hours, "",
            day(p.first), day(p.last), p.unrated],
  }));
  const t = table(ws,
    ["Person", "Role", "Logs", "Hours", "Share of the hours",
     "First log", "Last log", "Hours not costed"],
    rows,
    { right: [3, 4, 5, 8], formats: [null, null, INT, HRS, PCT, dmy, dmy, HRS],
      total: [3, 4, 8] });

  // La quota si scrive come formula sulla riga del totale, così regge anche se
  // qualcuno filtra o riordina il foglio.
  if (t.total) {
    for (let r = t.first; r <= t.last; r++) {
      const c = ws.getCell(r, 5);
      c.value = { formula: `IF($D$${t.total}=0,0,D${r}/$D$${t.total})` };
      c.numFmt = PCT;
    }
    const tc = ws.getCell(t.total, 5);
    tc.value = { formula: `IF($D$${t.total}=0,0,1)` };
    tc.numFmt = PCT;
  }
  ws.__r++;
  note(ws, "Hours come from the Zoho Projects time logs, one row per person. What is not in this " +
           "file is the money: each person's hourly cost and the cost their hours generate are " +
           "part of the internal costs view.");
  note(ws, "\"Hours not costed\" are hours the margin prices at zero because the person has no " +
           "hourly cost on record — those rows are shaded, and the margin is flattered by that much.");
  ws.__r++;
  return { hoursRow: t.total, total: t.total, costCol: "D" };
}
...
export async function dealWorkbook(ctx, dealId, role) {
  ...
  const ws = sheet(wb, "Deal", deal.name,
    ...
    (viewer ? " Hours are broken down by person on the team sheet; their cost is part of the internal costs view." : ""));
  ...
  if (people.length) {
    ...
    (viewer ? teamHoursBlock : peopleBlock)(team, people);
  }
  return wb;
}
...
export async function clientWorkbook(ctx, clientName, role) {
  ...
  if (people.length) {
    ...
    (viewer ? teamHoursBlock : peopleBlock)(tws, people);
  }
  return wb;
}
...
export async function dashboardWorkbook(ctx, view, role) {
  ...
  note(ws, "Margin is before server and infrastructure costs, which Zoho does not record." +
           (viewer
             ? " Hours are shown person by person; what the internal costs view adds is the money — each person's hourly cost and the cost of the team."
