...
export const INTERNAL_SQL = (() => {
  ...
  // Espressione semplice, non aggregata: finisce sia nella SELECT sia nella
  // GROUP BY, e un aggregato lì non si può mettere.
  return `CASE WHEN ${clean('T."Tasklist Name"')} = '${wanted}' ` +
         `OR ${clean('T."Task Name"')} = '${wanted}' THEN 1 ELSE 0 END`;
})();
...
export async function fetchProjectCosts() {
  ...
  const sql =
    ...
    `YEAR(L."${c.date}") AS yr, MONTH(L."${c.date}") AS mo, ${INTERNAL_SQL} AS internal, ` +
    `SUM(L."${c.hours}")*1 AS hours, ` +
    `SUM(L."${c.hours}"*L."${rate}")*1 AS zoho_cost, ` +
    `SUM(CASE WHEN L."${rate}" > 0 THEN 0 ELSE L."${c.hours}" END)*1 AS unrated ` +
    `FROM "${TIME_LOGS}" L ` +
    `LEFT JOIN "${TASKS}" T ON CONCAT(T."Task ID",'') = CONCAT(L."Task ID",'') ` +
    `WHERE YEAR(L."${c.date}") >= ${from} ` +
    `GROUP BY L."${c.project}", L."User ID", YEAR(L."${c.date}"), MONTH(L."${c.date}"), ${INTERNAL_SQL}`;
...
export async function runCostQueryRaw(sql) {
