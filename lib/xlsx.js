...
function projectListBlock(ws, projects, heading, opts) {
  ...
  const t = o.viewer
    ? table(ws,
        ["Project", "Status", "Link to deal", "Deal", "Hours",
         "Deal revenue (€)", "Margin (€)", "Margin %"],
        rows,
        { right: [5, 6, 7, 8],
          formats: [null, null, null, null, HRS, EUR, EUR, PCT],
          total: [5] })
    : table(ws,
...
export async function dashboardWorkbook(ctx, view, role) {
  ...
  head.push("Hours", `Margin — ${yl} (€)`, `Margin % — ${yl}`);
  fmts.push(HRS, EUR, PCT);
  ...
  head.push("Lifetime value (€)", "Won deals", "Invoices", "Outstanding (€)", "Projects");
  fmts.push(EUR, INT, INT, EUR, INT);
  ...
  const t = table(ws, head,
    ...
    { right: head.map((_, i) => i + 1).filter((i) => i > 2),
      ...
      total: head.map((h, i) => (/%|Share/.test(h) || i < 2 ? null : i + 1)).filter(Boolean) });
